...
export default function AuditTrailExport() {
  ...
  const filteredLogs = logs?.filter((log: any) => {
    ...
    // User filter
    if (userFilter && log.userId !== parseInt(userFilter)) {
      return false;
    }
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      // For now, export as CSV with .xlsx extension (proper XLSX would need xlsx library on frontend)
      const csvContent = [
        headers.join("\t"),
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      // Create PDF content (simplified - would use proper PDF library in production)
      let pdfContent = `
AUDIT TRAIL REPORT
==================
