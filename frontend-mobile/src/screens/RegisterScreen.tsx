  ...
  const uploadDataToBackend = async () => {
    ...
    try {
      ...
      if (isSuccessful) {
        ...
        // ✅ Save Access Token if available (Auto-Login)
        if (backendResult.accessToken) {
          try {
            await SecureStore.setItemAsync('accessToken', backendResult.accessToken);
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.pendingContainer}>
          <View style={styles.pendingContent}>
            <View style={styles.successIcon}>
              <Ionicons name="checkmark-sharp" size={50} color="#D97706" />
            </View>
            ...
            <View style={styles.infoCard}>
              <Text style={styles.infoText}>
                Your digital identity request has been received.{"\n"}
                We will notify you once approved.
              </Text>

              <Text style={[styles.infoText, { marginTop: 20 }]}>
                To receive notifications and 2FA codes, please link your Telegram account.
              </Text>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  Linking.openURL('https://t.me/DigitalID_Bot');
                }}
              >
                <MaterialCommunityIcons name={"telegram" as any} size={20} color="white" />
                <Text style={{ color: 'white', fontWeight: 'bold' }}>Connect Telegram</Text>
              </TouchableOpacity>

              <Text style={styles.infoTextId}>ID Ref: {extractedData?.id || "PENDING"}</Text>
            </View>
